...
function sumByJournal(rows, valueKey = "value") {
    const acc = {};
    for (const r of rows ?? []) {
        const j = String(r?.journal ?? "").trim();
        const v = Number(r?.[valueKey]);
        if (!j || !Number.isFinite(v))
            continue;
        acc[j] = (acc[j] ?? 0) + v;
    }
    return Object.entries(acc).map(([journal, count]) => ({ journal, count }));
}
function normJournalKey(name) {
    const n = String(name ?? "").toUpperCase();
    if (n.startsWith("JACC"))
        return "JACC";
    if (n.startsWith("CIRC"))
        return "CIRC"; // Circulation
    if (n.startsWith("EHJ") || n.includes("EUROPEAN"))
        return "EHJ";
    return n;
}
export async function getDefaultMetrics() {
    try {
        ...
        const { data } = await api.get("/dashboard/default");
        ...
        for (const r of (data?.impact_factors ?? [])) {
            ...
            const row = (ifByYear[year] || (ifByYear[year] = { year }));
            row[key] = val;
        }
