...
const PER_PAIR_FIELDS = new Set(FIELDS.map(f => f.name).filter(n => !CONDITION_AGG_FIELDS.has(n) && !AGG_ONLY_FIELDS.has(n)));
/* ----------------------- tiny helpers ----------------------- */
...
function csvList(field, csvOrArray) {
...
function condAggExprFor(field) {
    if (field === "citation_count")
        return "CAST(COUNT(pp.cited_eid) AS float)";
    if (field === "fitness")
        return "CAST(MAX(pp.cited_fitness) AS float)";
    // default: treat as per_pair (shouldn't happen)
    return `pp.${field}`;
}
// Field expression for each context
function fieldExpr(field, ctx) {
    if (ctx === "per")
        return `pp.${field}`;
    if (ctx === "condAgg")
        return condAggExprFor(field);
    // agg: columns are materialized in 'aggregation' CTE
    return `[${field}]`;
}
function ruleToSqlInCtx(r, ctx) {
    ...
    // text operators
    if (op === "contains")
        return `${f} LIKE ${quote(`%${String(v)}%`)}`;
    if (op === "doesNotContain")
        return `${f} NOT LIKE ${quote(`%${String(v)}%`)}`;
    if (op === "beginsWith")
        return `${f} LIKE ${quote(`${String(v)}%`)}`;
    ...
    // in / notIn
    if (op === "in")
        return `${f} IN (${csvList(field, v)})`;
    if (op === "notIn")
        return `${f} NOT IN (${csvList(field, v)})`;
    // null checks
    if (op === "isNull")
        return `${f} IS NULL`;
    if (op === "isNotNull")
...
function buildBoolean(qb, ctx, includeField) {
    if (!qb || !qb.rules)
        return "1=1";
    const walk = (g) => {
        const rules = g.rules ?? [];
        ...
        for (const r of rules) {
            if ("rules" in r) {
                ...
                if (!includeField(field))
                    continue;
                parts.push(ruleToSqlInCtx(r, ctx));
            }
        }
...
const includePerPair = (name) => PER_PAIR_FIELDS.has(name);
const includeCondAgg = (name) => CONDITION_AGG_FIELDS.has(name);
const includeAggOnly = (name) => AGG_ONLY_FIELDS.has(name);
/* ------------------------- main builder ------------------------- */
export function buildSql(qb, cfg) {
    ...
    const yExpr = cfg.yAgg === "COUNT" || cfg.yField === "n_article"
        ? "COUNT(*) AS n_articles"
